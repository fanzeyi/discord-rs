...
impl State {
	...
	pub fn all_private_channels(&self) -> Vec<ChannelId> {
		self.groups
			.keys()
			.cloned()
			.chain(self.private_channels.iter().map(|c| c.id))
			.collect()
	}
	...
	pub fn all_servers(&self) -> Vec<ServerId> {
		self.servers
			.iter()
			.map(|s| s.id)
			.chain(self.unavailable_servers.iter().cloned())
			.collect()
	}
	...
	pub fn update(&mut self, event: &Event) {
		match *event {
			...
			Event::UserSettingsUpdate {
				ref detect_platform_accounts,
				ref developer_mode,
				ref enable_tts_command,
				ref inline_attachment_media,
				ref inline_embed_media,
				ref locale,
					...
					match self.groups.entry(group.channel_id) {
						...
						Entry::Occupied(mut e) => {
							...
							if group.recipients.is_empty() {
								...
								let recipients =
									::std::mem::replace(&mut dest.recipients, Vec::new());
	...
	pub fn unavailable_servers(&self) -> &[ServerId] {
	...
	#[inline]
	pub fn groups(&self) -> &BTreeMap<ChannelId, Group> {
		&self.groups
	}
	...
	#[inline]
	pub fn calls(&self) -> &BTreeMap<ChannelId, Call> {
		&self.calls
	}
	...
	/// Returns `None` on bot accounts.
	#[inline]
	pub fn server_settings(&self) -> Option<&[UserServerSettings]> {
		self.server_settings.as_ref().map(|x| &x[..])
	}
